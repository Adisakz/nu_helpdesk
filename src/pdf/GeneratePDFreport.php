...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM repair_report_pd05 WHERE close BETWEEN '$start_date 00:00:00' AND '$end_date 23:59:59' AND status = 3";
...
if(isset($signature_director)){
  $pdf->Image("../image_signature/$signature_director", 54, 260, 20, 20);
...
if(isset($signature_dean)){
  ...
  $pdf->Cell(62, 5, iconv('utf-8', 'cp874', " $date_dean_update "), 0, 1, 'C');
    ...
    $pdf->Output();
        ...
        $pdf->SetFont('sara', 'B', 16);
        $pdf->Cell(0, 0, '', 0, 1); // เพิ่มช่องว่างเพื่อให้ข้อความอยู่ตรงกลาง

        $pdf->Cell(0, 10, iconv('utf-8', 'cp874', "ไม่พบข้อมูล"), 0, 1, 'C');
        $pdf->Output();
    }
}
?>