...
document.addEventListener('DOMContentLoaded', function() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = 'confirm_req_parcel';
        }
    });
});
</script>
...
<?php
        ...
        if (in_array($img_ex_lc, $allowed_exs)) {
            if ($img_size > 125000) {
                ...
                $sql1 = "UPDATE report_req_parcel SET signature_parcel_head='$resized_img_name', id_parcel_head='$id_person', status='3' WHERE id=$id_req_parcel";
                mysqli_query($conn, $sql1);
                header("location: ./confirm_req_parcel?success=success");
                ...
                $sql1 = "UPDATE report_req_parcel SET signature_parcel_head='$new_img_name', id_parcel_head='$id_person', status='3' WHERE id=$id_req_parcel";
                mysqli_query($conn, $sql1);
                header("location: ./confirm_req_parcel?success=success");
            }
        } else {
            echo "สกุลไม่ถูกต้อง";
            header("location: ./confirm_req_parcel?error=error");
        }
    }
}
    ...
    if (isset($_POST['signaturesubmit'])) {
      ...
      $id = $_POST['data1'];
      $id_person = $_SESSION['id'] ;
        $sql1 = "UPDATE report_req_parcel SET signature_parcel_head='$signatureFileName', id_parcel_head='$id_person' , status='3' WHERE id=$id";
        mysqli_query($conn, $sql1);
        header("location: ./confirm_req_parcel?success=success");
...
<body class="hold-transition sidebar-mini">
    <div class="wrapper">
        ...
        <script>
        document.addEventListener('DOMContentLoaded', function() {
            const typeParcelNavItem = document.querySelector('a[name="check-req-parcecl"]')
                                        ...
                                        <div class="card-footer">
                                            ...
                                            <center>
                                                ...
                                                <a class="btn btn-danger btn-sm" href="#"
                                                    onclick="showRejectionReasonPrompt(<?php echo $id_req_parcel; ?>)">ไม่อนุมัติ</a>
                                                <button type="submit" name="submit" class="btn btn-success btn-sm"><i
                                                        class="fas fa-pencil-alt"></i>อนุมัติ</button>
                                            </center>
