...
<?php
        ...
        if (in_array($img_ex_lc, $allowed_exs)) {
            if ($img_size > 125000) {
                ...
                // กำหนดค่าให้กับ $dir
                $dir = '../image_signature/' . $resized_img_name;
                ...
                foreach ($dataArray as $index => $item) {
                    ...
                    $sql_parcel_data = "SELECT * FROM parcel_data WHERE id_parcel = '$id_parcel' LIMIT 1";
                    $result_parcel_data = mysqli_query($conn, $sql_parcel_data);
                    $row1 = mysqli_fetch_assoc($result_parcel_data);
                    $max = $row1['qty'];
            
                    $quantity1 = $quantity[$index];
                    $update_qty =$max-$quantity1;
                    if ($update_qty < 0) {
                        $update_qty = 0;
                    }
                   
                   $sql = "UPDATE parcel_data SET qty= $update_qty WHERE id_parcel=$id_parcel";                   
                    mysqli_query($conn, $sql);
                }
                $sql1 = "UPDATE report_req_parcel SET signature_success='$resized_img_name' , status='4',close=CURRENT_TIMESTAMP WHERE id=$id_req_parcel";
                ...
                $sql1 = "UPDATE report_req_parcel SET signature_success='$new_img_name' , status='4',close=CURRENT_TIMESTAMP WHERE id=$id_req_parcel";
                mysqli_query($conn, $sql1);
                header("location: ./req_parcel_list?success=success");
            }
        } else {
          echo "สกุลไม่ถูกต้อง";
          header("location: ./req_parcel_list?error=error");
      }
        }
    }
    ...
    if (isset($_POST['signaturesubmit'])) {
      ...
      file_put_contents($file, $data);
        ...
        $sql1 = "UPDATE report_req_parcel SET  signature_success='$signatureFileName' , status='4',close=CURRENT_TIMESTAMP WHERE id=$id_req_form_value";
        mysqli_query($conn, $sql1);
        header("location: ./req_parcel_list?success=success");
                                        ...
                                        <table border="1">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center" hidden>จำนวนที่ให้เบิก</th>
                                                ...
                                                <?php
                                                   ...
                                                   while ($row = mysqli_fetch_assoc($result_req)) {
                                                       echo '<tr>
                                                               ...
                                                               <td class="text-center" hidden><input type="number" name="quantity[]" class="quantity" value="' . $row['qty'] . '" style="width: 50px; text-align: center; margin-left: auto;" min="0" max="' . $row['qty'] . '"></td>
                                        ...
                                        <div class="card-footer">
                                            ...
                                            <center>
                                                <input type="hidden" id="dataArrayInput" name="dataArrayInput"
                                                    ...
                                                    <a class="btn btn-danger btn-sm" href="javascript:history.go(-1);">ย้อนกลับ</a>
                                                <button type="submit" name="submit" class="btn btn-success btn-sm"><i
                                                        class="fas fa-pencil-alt"></i>รับพัสดุ</button>
                                            </center>
