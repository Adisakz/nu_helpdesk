...
$id_person_edit = $_GET['id'];
$sql_edit = "SELECT * FROM account WHERE id_person = $id_person_edit";
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="flex-container">
                                        <input type="text" name="urole" class="form-control" id="urole"
                                            ...
                                            <a class="btn btn-primary" onclick="showChangeUrolePopup()">เปลี่ยนสิทธิ์</a>
                                    </div>
                                </div>
...
<!-- Modal เปลี่ยนสิทธิ์-->
<div class="modal fade" id="changeUroleModal" tabindex="-1" role="dialog" aria-labelledby="changeUroleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="changeUroleModalLabel">เปลี่ยนสิทธิ์</h5>
            ...
            <input type="text" name="id-person" class="form-control" id="id-person"
                ...
                <select id="newUroleDropdown" class="form-control">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="saveNewUrole()">บันทึก</button>
            </div>
        </div>
    </div>
</div>
...
function saveNewDepartment() {
    ...
    function showChangeUrolePopup() {
        // แสดง Modal เปลี่ยนสิทธิ์
        $('#changeUroleModal').modal('show');

        // ดึงข้อมูลสิทธิ์เก่ามาแสดงใน dropdown
        var currentUrole = $('#urole').val();
        $('#newUroleDropdown').val(currentUrole);
    }

    function saveNewUrole() {
        var userId = $('#id-person').val(); // ดึงค่า id จาก input hidden
        var newUroleValue = $('#newUroleDropdown').val();

        $.ajax({
            url: './controller/edit_urole_profile.php', // เปลี่ยนเป็นไฟล์ที่ทำหน้าที่บันทึกข้อมูล
            method: 'POST',
            data: { 
                userId: userId, 
                urole: newUroleValue 
